...
namespace BoletoNet
{
    ...
    internal class Banco_Itau : AbstractBanco, IBanco
    {
        ...
        public override void ValidaBoleto(Boleto boleto)
        {
            ...
            {
                //Carteiras válidas
                int[] cv = new int[] { 175, 176, 178, 109, 198, 107, 122, 142, 143, 196, 121 /*, 126, 131, 146, 150, 168*/ };//Flavio(fhlviana@hotmail.com) - adicionado a carteira 109
                bool valida = false;
                ...
                if (boleto.Cedente.ContaBancaria.Conta.Length > 5)
                    throw new NotImplementedException("A quantidade de dígitos da Conta " + boleto.Cedente.ContaBancaria.Conta + ", são de 5 números.");
                else if (boleto.Cedente.ContaBancaria.Conta.Length < 5)
        ...
        public override void FormataCodigoBarra(Boleto boleto)
        {
            try
            {
                ...
                string codigoCedente = string.IsNullOrEmpty(boleto.Cedente.Codigo) ? string.Empty : Utils.FormatCode(boleto.Cedente.Codigo.ToString(), 5);
            ...
            try
            {
                ...
                if (boleto.Instrucoes.Count > 0)
                {
                    for (int i = 0; i < boleto.Instrucoes.Count; i++)
                    {
                        ...
                        {
                            _detalhe += boleto.Instrucoes[i].QuantidadeDias.ToString();
                            break;
                        }
